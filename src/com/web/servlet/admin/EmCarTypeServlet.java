...
public class EmCarTypeServlet extends UserSecureDispatcher {

	private static final long serialVersionUID = 1488465993112L;
	...
	public void blank(HttpServletRequest request, HttpServletResponse response)
		...
		forward(request, response, "/admin/CarTypeAdd.jsp");
	...
	public void add(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			String cartypename = Putil.getString(request.getParameter("cartypename"));
			...
			StringBuilder select = new StringBuilder("insert into em_cartype (cartypename,photo,photo1,price,seq) values ("
				+ "'" + cartypename.replace("'", "''") + "'"
		...
		redirect(request, response, "/emCarType.do?method=list&uf_parentid="+uf_parentid+"&keyword="+keyword+"&m="+m+"&s="+s);
	...
	public void fill(HttpServletRequest request, HttpServletResponse response)
		...
		String cartypeid = Putil.getString(request.getParameter("cartypeid")) ;
		if(cartypeid.length()>0) {
			Map<String, Object> row = DbUtils.queryOne("select p.* from em_cartype p where p.cartypeid="+cartypeid);
			request.setAttribute("item", row);
		}

		forward(request, response, "/admin/CarTypeEdit.jsp");
	...
	public void update(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder select = new StringBuilder("update em_cartype set "
					+ "cartypename='" + cartypename.replace("'", "''") + "'"
					+ (fileSuc0?",photo = '" + filename0 + "'":"")
					+ (fileSuc1?",photo1 = '" + filename1 + "'":"")
					+ ",price=" + price + ""
					+ ",seq=" + seq + ""
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder countSql = new StringBuilder("select count(p.cartypeid) as total from em_cartype p where p.cartypeid>=0"
					+ (keyword.length()>0?" and p.cartypename like '%" + keyword + "%'":"")
					);
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_cartype p where p.cartypeid>=0"
					+ (keyword.length()>0?" and p.cartypename like '%" + keyword + "%'":"")
		...
		forward(request, response, "/admin/CarTypeList.jsp");
	...
	public void delete(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder select = new StringBuilder("delete from em_cartype "
					+ " where cartypeid=" + cartypeid + "" 
					);
