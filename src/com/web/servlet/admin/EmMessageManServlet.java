...
public class EmMessageManServlet extends UserSecureDispatcher {
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("user", LoginManager.checkUserLogin(request)) ;
		...
		try {
			...
			List<Map<String, Object>> adminList = DbUtils.query("select p.*,a.authorityno from em_user p,em_authority a where p.authorityid=a.authorityid"
					+ " and p.isvalid=1"
					);
			if(adminList!=null && adminList.size()>0) {
				for(Map<String, Object> a:adminList) {
					if(LoginManager.checkUserAuthority(51 ,Putil.getString(a.get("authorityno")))) {
						aList.add(a);
					}
				}
			}
			...
			StringBuilder countSql = new StringBuilder("select count(p.userid) as total from em_user p where p.userid>=0 and p.authorityid=0"
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_user p where p.userid>=0 and p.authorityid=0"
					+ (keyword.length()>0?" and (p.username like '%" + keyword + "%' or p.account like '%" + keyword + "%' or p.mobile like '%" + keyword + "%')":"")
					+ " order by p.userid desc limit " + (s-1)*m + "," + m + ""
				);
		...
		forward(request, response, "/admin/MessageManList.jsp");
