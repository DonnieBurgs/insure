...
/*
	<servlet>
		...
		<servlet-class>com.insure.servlet.admin.EmGroupInsurancePolicyServlet</servlet-class>
	</servlet>
	<servlet-mapping>
		<servlet-name>emGroupInsurancePolicy</servlet-name>
...
public class EmGroupInsurancePolicyServlet extends UserSecureDispatcher {
	...
	private static final long serialVersionUID = 1491580695249L;
	...
	public void fill(HttpServletRequest request, HttpServletResponse response)
		...
		if(id.length()>0) {
			Map<String, Object> row = DbUtils.queryOne("select p.* from em_groupinsurancepolicy p where p.id="+id);
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder countSql = new StringBuilder("select count(p.id) as total from em_groupinsurancepolicy p where p.id>=0"
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_groupinsurancepolicy p where p.id>=0"
					+ (keyword.length()>0?" and p.groupinsurancepolicyname like '%" + keyword + "%'":"")
