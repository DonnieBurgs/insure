...
/*
	<servlet>
		...
		<servlet-class>com.insure.servlet.admin.EmDiseaseServlet</servlet-class>
	</servlet>
	<servlet-mapping>
		<servlet-name>emDisease</servlet-name>
...
public class EmDiseaseServlet extends UserSecureDispatcher {
	...
	private static final long serialVersionUID = 1491580695667L;
	...
	public void fill(HttpServletRequest request, HttpServletResponse response)
		...
		if(id.length()>0) {
			Map<String, Object> row = DbUtils.queryOne("select p.* from em_disease p where p.id="+id);
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder countSql = new StringBuilder("select count(p.id) as total from em_disease p where p.id>=0"
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_disease p where p.id>=0"
					+ (keyword.length()>0?" and p.diseasename like '%" + keyword + "%'":"")
    ...
    public void def(HttpServletRequest request, HttpServletResponse response)
		...
		List<Map<String, Object>> resultRows = new ArrayList<Map<String, Object>>();
		StringBuilder sql = new StringBuilder("select p.* from em_disease p where p.id>=0");
		if(keyword.length() > 0){
			sql.append(" and (p.diseasename like '%" + keyword + "%'");
			sql.append(" or p.diseasecode like '%" + keyword + "%'");
