...
String key_areaid = (String)request.getAttribute("key_areaid");
String key_areaid1 = (String)request.getAttribute("key_areaid1");
String uf_parentid = (String)request.getAttribute("uf_parentid");
if(keyword==null) keyword = "";
if(key_areaid==null) key_areaid = "";
if(key_areaid1==null) key_areaid1 = "";
...
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">医院列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth42 eq '1'}"><a href="/emHospital.do?method=blank&uf_parentid=${uf_parentid}&keyword=${keyword}&key_areaid=${key_areaid}&key_areaid1=${key_areaid1}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增医院</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>
<table class="tlist">
	 <form action="/emHospital.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>地区</th>
	 <th width=30% align=left style="background-color:#ffffff;text-align:left;">
			<select name="key_areaid1" onchange="loadArea(this, key_areaid)" style="width:110px;">
	 		<option value="">－－全部－－</option>
	 		 <c:if test="${not empty areaList1}">
	 		 <c:forEach var="area" items="${areaList1}" varStatus="idx">
	 		 		<option value="${area.areaid}"<c:if test="${key_areaid1 eq area.areaid}"> selected</c:if>>${area.areaname}</option>
	 		 </c:forEach>
	 		 </c:if>
	 		</select>
			<select name="key_areaid" style="width:110px;">
	 		<option value="">－－全部－－</option>
	 		 <c:if test="${not empty areaList2}">
	 		 <c:forEach var="area" items="${areaList2}" varStatus="idx">
	 		 		<option value="${area.areaid}"<c:if test="${key_areaid eq area.areaid}"> selected</c:if>>${area.areaname}</option>
	 		 </c:forEach>
	 		 </c:if>
	 		</select>
	 </th>
...
<td>${item.addr}</td>
				...
				<c:if test="${auth42 eq '1'}"><a href="/emHospital.do?method=fill&hospitalid=${item.hospitalid}&keyword=${keyword}&key_areaid=${key_areaid}&key_areaid1=${key_areaid1}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a>
					<a href="javascript:deleteHospital(${item.hospitalid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a>
				</c:if>
				<c:if test="${auth42 ne '1'}"><a href="/emHospital.do?method=fill&hospitalid=${item.hospitalid}&keyword=${keyword}&key_areaid=${key_areaid}&key_areaid1=${key_areaid1}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emHospital.do?method=list&keyword=" + keyword + "&key_areaid=" + key_areaid + "&key_areaid1=" + key_areaid1 + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emHospital.do?method=list&keyword=" + keyword + "&key_areaid=" + key_areaid + "&key_areaid1=" + key_areaid1 + "&uf_parentid=" + uf_parentid;
...
<form action="/emHospital.do?method=delete" id="deleteForm" name="deleteForm" method="post">
