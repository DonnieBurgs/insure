...
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">会员列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth46 eq '1'}"><a href="/emUser.do?method=blank&uf_parentid=${uf_parentid}&key_isvalid=${key_isvalid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增会员</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>

<table class="tlist">
	 <form action="/emUser.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 	<th width=10%>关键字</th>
...
<option value="1"<c:if test="${key_isvalid eq 1}"> selected</c:if>>有效
	 	...
	 	<th width="20%" class="operationBtn">
	 		...
	 		<a href="javascript:submitform(1);" style="margin-left:10px;"><i class="iconfont">&#xe64a;</i>导出</a>
	 	</th>
	 </tr>
	 </thead>
	 <tbody>
...
<c:forEach var="item" items="${resultRows}">
			<tr>
				<td height=30>${item.userid}</td>
				<td height=30>${item.username}</td>
				<td>${item.mobile}</td>
				<td>${item.sex}</td>
				<td><c:if test="${item.isdelete eq 1}">已删除</c:if><c:if test="${item.isdelete ne 1}">有效</c:if></td>
				<td width="700" class="operationBtn">
				<c:if test="${auth46 eq '1'}"><a href="/emUser.do?method=fill&userid=${item.userid}&uf_parentid=${uf_parentid}&key_isvalid=${key_isvalid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a></c:if>
				<c:if test="${auth46 eq '1'}"><a href="/emOrder.do?method=list&userid=${item.userid}&uf_parentid=${uf_parentid}&key_isvalid=${key_isvalid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6bc;</i>订单</a></c:if>
				<c:if test="${auth46 eq '1'}"><a href="/emCoupon.do?method=list&userid=${item.userid}&uf_parentid=${uf_parentid}&key_isvalid=${key_isvalid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6c8;</i>救援金券</a></c:if>
				<c:if test="${auth46 eq '1'}"><a href="/emDeposits.do?method=list&userid=${item.userid}&uf_parentid=${uf_parentid}&key_isvalid=${key_isvalid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe609;</i>预存款</a></c:if>
				<c:if test="${auth46 eq '1'}"><a href="/emWithdraw.do?method=list&userid=${item.userid}&uf_parentid=${uf_parentid}&key_isvalid=${key_isvalid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6a5;</i>提现</a></c:if>
				<c:if test="${auth46 eq '1'}"><c:if test="${item.isdelete eq 0 }"><a href="javascript:deleteUser(${item.userid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a></c:if></c:if>

				<c:if test="${auth46 ne '1'}"><a href="/emUser.do?method=fill&userid=${item.userid}&uf_parentid=${uf_parentid}&key_isvalid=${key_isvalid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emUser.do?method=list&uf_parentid=" + uf_parentid + "&key_isvalid=" + key_isvalid + "&keyword=" + keyword + "&m=" + pageSize;
			String u1 = "/emUser.do?method=list&uf_parentid=" + uf_parentid + "&key_isvalid=" + key_isvalid + "&keyword=" + keyword;
...
<form action="/emUser.do?method=delete" id="deleteForm" name="deleteForm" method="post">
