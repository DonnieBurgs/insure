...
<title>交通工具类型列表</title>
</head>
<body id="right">
<div class="mtitle ui-widget-header">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">交通工具类型列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth42 eq '1'}"><a href="/emTransType.do?method=blank&uf_parentid=${uf_parentid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增交通工具类型</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>
<table class="tlist">
	 <form action="/emTransType.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>关键字</th>
...
<td>${item.seq}</td>
				...
				<c:if test="${auth42 eq '1'}"><a href="/emTransType.do?method=fill&transtypeid=${item.transtypeid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a>
					<a href="javascript:deleteTransType(${item.transtypeid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a>
				</c:if>
				<c:if test="${auth42 ne '1'}"><a href="/emTransType.do?method=fill&transtypeid=${item.transtypeid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emTransType.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emTransType.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid;
...
<form action="/emTransType.do?method=delete" id="deleteForm" name="deleteForm" method="post">
<input type="hidden" id="transtypeid" name="transtypeid" value="">
...
function deleteTransType(transtypeid){
	if(confirm("是否删除会员资料？")) {
		$("#transtypeid").val(transtypeid);
		$("#t").val(t);
		deleteForm.submit();
