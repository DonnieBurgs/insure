...
<title>救援金券<c:if test="${auth70 eq '1'}">修改</c:if><c:if test="${auth70 ne '1'}">查看</c:if></title>
</head>
<body id="right">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">救援金券<c:if test="${auth70 eq '1'}">修改</c:if><c:if test="${auth70 ne '1'}">查看</c:if></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:history.back();" style="margin-left:100px;"><i class="iconfont">&#xe63d;</i>返回列表</a></div>
	<div class="operationRight f-r">
	</div>
</div>
<form id="commonForm" action="/emCoupon.do?method=update" method="post">
	<table class="tform">
<tr>
<td width="120" class="right">救援金券名称：</td>
<td><input type="text" id="couponname" name="couponname" value="${fn:replace(item.couponname,'"','&quot;')}" size=50></td>
</tr>
<tr>
...
<td><input type="text" id="count" name="count" value="${fn:replace(item.count,'"','&quot;')}" size=50></td>
</tr>
<tr>
<td width="120" class="right">有效时间：</td>
<td><input type="text" id="validstartdate" name="validstartdate" readonly="readonly" value="${item.validstartdate_}" onclick="WdatePicker({maxDate:'2018-12-31', dateFmt:'yyyy-MM-dd'})" style="width: 70px;"/></td>
</tr>
<tr>
<td width="120" class="right">有效时间：</td>
<td><input type="text" id="validenddate" name="validenddate" readonly="readonly" value="${item.validenddate_}" onclick="WdatePicker({maxDate:'2018-12-31', dateFmt:'yyyy-MM-dd'})" style="width: 70px;"/></td>
</tr>
<tr>
<td width="120" class="right">简介：</td>
<td><textarea id="note" name="note" style="width:400px;height:60px;padding:5px;">${fn:replace(item.note,'"','&quot;')}</textarea></td>
</tr>
<tr>
...
<td><input type="text" id="seq" name="seq" value="${fn:replace(item.seq,'"','&quot;')}" size=50></td>
		...
		<tr>
			<td colspan="2">
			<input id="couponid" name="couponid" type="hidden" value="${item.couponid}"/>
...
<form id="deleteForm" action="/emCoupon.do?method=delete" method="post">
<input id="couponid" name="couponid" type="hidden" value="${item.couponid}"/>
<input id="uf_parentid" name="uf_parentid" type="hidden" value="${uf_parentid}"/>
<input id="keyword" name="keyword" type="hidden" value="${keyword}"/>
<input id="m" name="m" type="hidden" value="${m}"/>
