...
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">专科类型列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth42 eq '1'}"><a href="/emSpecialty.do?method=blank&uf_parentid=${uf_parentid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增专科类型</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>
<table class="tlist">
	 <form action="/emSpecialty.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>关键字</th>
...
<td>${item.seq}</td>
				...
				<c:if test="${auth42 eq '1'}"><a href="/emSpecialty.do?method=fill&specialtyid=${item.specialtyid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a>
					<a href="javascript:deleteSpecialty(${item.specialtyid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a>
				</c:if>
				<c:if test="${auth42 ne '1'}"><a href="/emSpecialty.do?method=fill&specialtyid=${item.specialtyid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emSpecialty.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emSpecialty.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid;
...
<form action="/emSpecialty.do?method=delete" id="deleteForm" name="deleteForm" method="post">
