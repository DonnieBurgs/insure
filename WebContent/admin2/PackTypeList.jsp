...
<body id="right">
<div class="mtitle ui-widget-header">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">套餐类型列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth42 eq '1'}"><a href="/emPackType.do?method=blank&uf_parentid=${uf_parentid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增套餐类型</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>
<table class="tlist">
	 <form action="/emPackType.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>关键字</th>
...
<td>${item.photo}</td>
<c:if test="${fn:length(cartypeList)>0}"><td>${item.price1}</td></c:if>
<c:if test="${fn:length(cartypeList)>1}"><td>${item.price2}</td></c:if>
<c:if test="${fn:length(cartypeList)>2}"><td>${item.price3}</td></c:if>
<c:if test="${fn:length(cartypeList)>3}"><td>${item.price4}</td></c:if>
<c:if test="${fn:length(cartypeList)>4}"><td>${item.price5}</td></c:if>
<td>${item.seq}</td>

				<td width="250" class="operationBtn">
				<c:if test="${auth42 eq '1'}"><a href="/emPackType.do?method=fill&packtypeid=${item.packtypeid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a>
					<a href="javascript:deletePackType(${item.packtypeid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a>
				</c:if>
				<c:if test="${auth42 ne '1'}"><a href="/emPackType.do?method=fill&packtypeid=${item.packtypeid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emPackType.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emPackType.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid;
...
<form action="/emPackType.do?method=delete" id="deleteForm" name="deleteForm" method="post">
<input type="hidden" id="packtypeid" name="packtypeid" value="">
...
function deletePackType(packtypeid){
	if(confirm("是否删除会员资料？")) {
		$("#packtypeid").val(packtypeid);
		$("#t").val(t);
		deleteForm.submit();
