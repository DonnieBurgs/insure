...
String key_otype = (String)request.getAttribute("key_otype");
String key_status = (String)request.getAttribute("key_status");
String uf_parentid = (String)request.getAttribute("uf_parentid");
if(keyword==null) keyword = "";
if(key_otype==null) key_otype = "";
if(key_status==null) key_status = "";
...
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">救援订单列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth12 eq '1'}"><a href="/emOrder.do?method=blank&uf_parentid=${uf_parentid}&keyword=${keyword}&key_otype=${key_otype}&key_status=${key_status}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增救援订单</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>
<table class="tlist">
	 <form action="/emOrder.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>关键字</th>
	 ...
	 <th width=10%>订单类别</th>
	 <th width=20% align=left style="background-color:#ffffff;text-align:left;">
	 	<select name="key_otype">
	<option value="">--全部--</option>
<c:if test="${not empty otList}">
<c:forEach var="listItem" items="${otList}" varStatus="idx">
	<option value="${listItem.ordertypeid}"<c:if test="${key_otype != '' && key_otype eq listItem.ordertypeid}"> selected</c:if>>${listItem.ordertypename}</option>
</c:forEach>
</c:if>
</select>
	 </th>
...
<c:forEach var="listItem" items="${orderstatusList}" varStatus="idx">
	<option value="${listItem.orderstatusid}"<c:if test="${key_status != '' && key_status eq listItem.orderstatusid}"> selected</c:if>>${listItem.orderstatusname}</option>
</c:forEach>
</c:if>
</select>
	 </th>
...
<td>${item.orderstatusname}</td>
				...
				<c:if test="${auth12 eq '1'}"><a href="/emOrder.do?method=fill&orderid=${item.orderid}&keyword=${keyword}&key_otype=${key_otype}&key_status=${key_status}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/处理</a>
				</c:if>
				<c:if test="${auth12 ne '1'}"><a href="/emOrder.do?method=fill&orderid=${item.orderid}&keyword=${keyword}&key_otype=${key_otype}&key_status=${key_status}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emOrder.do?method=list&keyword=" + keyword + "&key_otype=" + key_otype + "&key_status=" + key_status + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emOrder.do?method=list&keyword=" + keyword + "&key_otype=" + key_otype + "&key_status=" + key_status + "&uf_parentid=" + uf_parentid;
...
<form action="/emOrder.do?method=delete" id="deleteForm" name="deleteForm" method="post">
