...
<title>救护车类型<c:if test="${auth42 eq '1'}">修改</c:if><c:if test="${auth42 ne '1'}">查看</c:if></title>
</head>
<body id="right">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">救护车类型<c:if test="${auth42 eq '1'}">修改</c:if><c:if test="${auth42 ne '1'}">查看</c:if></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:history.back();" style="margin-left:100px;"><i class="iconfont">&#xe63d;</i>返回列表</a></div>
	<div class="operationRight f-r">
	</div>
</div>
<form id="commonForm" action="/emCarType.do?method=update" method="post">
	<table class="tform">
<tr>
<td width="120" class="right">救护车类型名称：</td>
<td><input type="text" id="cartypename" name="cartypename" value="${fn:replace(item.cartypename,'"','&quot;')}" size=50></td>
</tr>
<tr>
<td width="120" class="right">图标(灰)：</td>
...
<tr>
<td width="120" class="right">图标(高亮)：</td>
<td><a class="mya" href="javascript:f_photo1.click();"><img id="imgphoto1" src="<c:if test="${empty item.photo1}">/resources/images/folder2.png</c:if><c:if test="${not empty item.photo1}">/upload/images/${item.photo1}</c:if>"></a></td>
</tr>
<tr>
...
<td><input type="text" id="seq" name="seq" value="${fn:replace(item.seq,'"','&quot;')}" size=50></td>
		...
		<tr>
			<td colspan="2">
			<input id="cartypeid" name="cartypeid" type="hidden" value="${item.cartypeid}"/>
...
<form id="deleteForm" action="/emCarType.do?method=delete" method="post">
<input id="cartypeid" name="cartypeid" type="hidden" value="${item.cartypeid}"/>
<input id="uf_parentid" name="uf_parentid" type="hidden" value="${uf_parentid}"/>
<input id="keyword" name="keyword" type="hidden" value="${keyword}"/>
<input id="m" name="m" type="hidden" value="${m}"/>
...
function checkf() {
	if(commonForm.cartypename.value=="") {alert("请正确输入救护车类型名称！");return false;}
