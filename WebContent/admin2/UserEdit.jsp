...
<title>会员<c:if test="${auth52 eq '1'}">修改</c:if><c:if test="${auth52 ne '1'}">查看</c:if></title>
</head>
<body id="right">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">会员 <c:if test="${auth52 eq '1'}">修改</c:if><c:if test="${auth52 ne '1'}">查看</c:if></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<tr>
<td width="120" class="right">手机（账号）：</td>
...
<td><textarea id="note" name="note" cols=80 rows=3>${item.note}</textarea></td>
		...
		<tr>
			<td colspan="2">
				...
				<input id="key_isvalid" name="key_isvalid" type="hidden" value="${key_isvalid}"/>
