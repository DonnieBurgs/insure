...
<title>配套服务类型<c:if test="${auth42 eq '1'}">修改</c:if><c:if test="${auth42 ne '1'}">查看</c:if></title>
</head>
<body id="right">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">配套服务类型<c:if test="${auth42 eq '1'}">修改</c:if><c:if test="${auth42 ne '1'}">查看</c:if></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:history.back();" style="margin-left:100px;"><i class="iconfont">&#xe63d;</i>返回列表</a></div>
	<div class="operationRight f-r">
	</div>
</div>
<form id="commonForm" action="/emServiceOptionType.do?method=update" method="post">
	<table class="tform">
<tr>
<td width="120" class="right">配套服务类型名称：</td>
<td><input type="text" id="serviceoptiontypename" name="serviceoptiontypename" value="${fn:replace(item.serviceoptiontypename,'"','&quot;')}" size=50></td>
</tr>
<tr>
...
<td><input type="text" id="seq" name="seq" value="${fn:replace(item.seq,'"','&quot;')}" size=50></td>
		...
		<tr>
			<td colspan="2">
			<input id="serviceoptiontypeid" name="serviceoptiontypeid" type="hidden" value="${item.serviceoptiontypeid}"/>
...
<form id="deleteForm" action="/emServiceOptionType.do?method=delete" method="post">
<input id="serviceoptiontypeid" name="serviceoptiontypeid" type="hidden" value="${item.serviceoptiontypeid}"/>
<input id="uf_parentid" name="uf_parentid" type="hidden" value="${uf_parentid}"/>
<input id="keyword" name="keyword" type="hidden" value="${keyword}"/>
<input id="m" name="m" type="hidden" value="${m}"/>
