...
<title>资讯<c:if test="${auth60 eq '1'}">修改</c:if><c:if test="${auth60 ne '1'}">查看</c:if></title>
</head>
<body id="right">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">资讯<c:if test="${auth60 eq '1'}">修改</c:if><c:if test="${auth60 ne '1'}">查看</c:if></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:history.back();" style="margin-left:100px;"><i class="iconfont">&#xe63d;</i>返回列表</a></div>
	<div class="operationRight f-r">
	</div>
</div>
<form id="commonForm" action="/emInformation.do?method=update" method="post">
	<table class="tform">
<tr>
...
<form id="deleteForm" action="/emInformation.do?method=delete" method="post">
<input id="informationid" name="informationid" type="hidden" value="${item.informationid}"/>
<input id="uf_parentid" name="uf_parentid" type="hidden" value="${uf_parentid}"/>
<input id="keyword" name="keyword" type="hidden" value="${keyword}"/>
<input id="m" name="m" type="hidden" value="${m}"/>
