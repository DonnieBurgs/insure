...
var validatePrompt={
	username:{
		...
		isNull:"请输入用户名",
		error:{
			beUsed:"该用户名已被使用，请使用其它用户名注册，如果您是&quot;{1}&quot;，请<a href='/user/login.jsp' class='flk13'>登录</a>",
	...
	mobile:{
		onFocus:"请输入常用的手机号码，将用来找回密码、接收订单通知等",
		succeed:"",
		isNull:"请输入手机号码",
		error:{
			beUsed:"该手机号码已被使用，请更换其它手机号码",
			badFormat:"手机号码格式不正确",
			badLength:"您填写的手机号码过长，11位有效数字"
		}
	},
	zipCode:{
		onFocus:"请输入邮政编码",
...
var nameold,emailold,authcodeold, mobileold;
var namestate=false,emailstate=false,mobilestate=false,authcodestate=false;
//回调函数
var validateFunction={
		...
		if(!length&&format){
	        ...
	        if(!namestate||nameold!=option.value)
	        {
	            if(nameold!=option.value)
	            {
	                ...
	                $.ajax({type: "get", cache: false, async: false, url: "/userServlet.do?method=checkname", data: {username: escape(option.value)}, dataType: "json", success: function(data){
	                	if(data.success==0){
		...
		if(!format){
			...
			if (!format2){
				...
				if(emailold==option.value){
			           validateSettings.succeed.run(option);
				}else if(!emailstate||emailold!=option.value)
		        {
		            if(emailold!=option.value)
		            {
		                option.errorEle.html("<span style='color:#999'>检验中……</span>");
		                $.ajax({type: "get", cache: false, async: false, url: "/userServlet.do?method=checkemail", data: {email: escape(option.value)}, dataType: "json", success: function(data){
		                	if(data.success==0){
				                validateSettings.succeed.run(option);
				                emailstate=true;
			                }else{
	...
	mobile:function(option){
		...
		if(!format){
			...
			if(!mobilestate||mobileold!=option.value)
	        {
	            if(mobileold!=option.value)
	            {
	                option.errorEle.html("<span style='color:#999'>检验中……</span>");
	                $.ajax({type: "get", cache: false, async: false, url: "/userServlet.do?method=checkmobile", data: {mobile: option.value}, dataType: "json", success: function(data){
	                	if(data.success==0){
			                validateSettings.succeed.run(option);
			                mobilestate=true;
		                }else{
	...
	zipCode:function(option){
		var format=new RegExp(validateRegExp.intege1).test(option.value);
		if(!format){
			validateSettings.error.run(option,option.prompts.error.badFormat);
