...
<table class="tlist">
	 ...
	 <th width=30% align=left style="background-color:#ffffff;text-align:left;">
	 	...
	 	<input type="text" id="emClaimArchive" name="emClaimArchive" value="${emClaimArchive }">
	 </th><th width=10%>姓名或证件号码</th>
	 <th width=30% align=left style="background-color:#ffffff;text-align:left;">
	 	<input type="hidden" id="insuredid" name="insuredid" value="${insuredid}">
	 	<input type="text" id="emInsured" name="emInsured" value="${emInsured }">
	 </th>
...
<td><dict:itemdesc name="idnumber" value="${item.insuredid}" table="em_insured" path="id"/></td>
<td></td><td><c:if test="${auth44 eq '1'}">
				<a href="/emClaimImage.do?method=blank&claimid=${item.id}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>上传影像件</a>
				</c:if></td>
			</tr>
</c:forEach>
</c:when>
