...
/*
	<servlet>
		...
		<servlet-class>com.web.servlet.admin.EmInsuranceServlet</servlet-class>
	</servlet>
	<servlet-mapping>
		<servlet-name>emInsurance</servlet-name>
...
public class EmInsuranceServlet extends UserSecureDispatcher {
	...
	private static final long serialVersionUID = 1491449283754L;
	
	public void blank(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String auth30 = request.getAttribute("auth30");
    	if(auth30.equals("")) {
	...
	public void fill(HttpServletRequest request, HttpServletResponse response)
		...
		String insuranceid = Putil.getString(request.getParameter("insuranceid")) ;
		if(insuranceid.length()>0) {
			Map<String, Object> row = DbUtils.queryOne("select p.* from em_insurance p where p.insuranceid="+insuranceid);
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder countSql = new StringBuilder("select count(p.insuranceid) as total from em_insurance p where p.insuranceid>=0"
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_insurance p where p.insuranceid>=0"
					+ (keyword.length()>0?" and p.insurancename like '%" + keyword + "%'":"")
