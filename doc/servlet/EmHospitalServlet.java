...
public class EmHospitalServlet extends UserSecureDispatcher {
	...
	private static final long serialVersionUID = 1491555808529L;
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder countSql = new StringBuilder("select count(p.hospitalid) as total from em_hospital p where p.hospitalid>=0"
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_hospital p where p.hospitalid>=0"
					+ (keyword.length()>0?" and p.hospitalname like '%" + keyword + "%'":"")
