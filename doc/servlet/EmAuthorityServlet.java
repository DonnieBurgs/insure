...
/*
	<servlet>
		...
		<servlet-class>com.web.servlet.admin.EmAuthorityServlet</servlet-class>
	</servlet>
	<servlet-mapping>
		<servlet-name>emAuthority</servlet-name>
...
public class EmAuthorityServlet extends UserSecureDispatcher {
	...
	private static final long serialVersionUID = 1491449284355L;
	
	public void blank(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String auth84 = request.getAttribute("auth84");
    	if(auth84.equals("")) {
	...
	public void fill(HttpServletRequest request, HttpServletResponse response)
		...
		if(authorityid.length()>0) {
			Map<String, Object> row = DbUtils.queryOne("select p.* from em_authority p where p.authorityid="+authorityid);
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder countSql = new StringBuilder("select count(p.authorityid) as total from em_authority p where p.authorityid>=0"
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_authority p where p.authorityid>=0"
					+ (keyword.length()>0?" and p.authorityname like '%" + keyword + "%'":"")
