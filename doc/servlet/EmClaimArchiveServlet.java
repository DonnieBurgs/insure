...
/*
	<servlet>
		...
		<servlet-class>com.web.servlet.admin.EmClaimArchiveServlet</servlet-class>
	</servlet>
	<servlet-mapping>
		<servlet-name>emClaimArchive</servlet-name>
...
public class EmClaimArchiveServlet extends UserSecureDispatcher {
	...
	private static final long serialVersionUID = 1491449283901L;
	
	public void blank(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		String auth42 = request.getAttribute("auth42");
    	if(auth42.equals("")) {
	...
	public void fill(HttpServletRequest request, HttpServletResponse response)
		...
		if(claimarchiveid.length()>0) {
			Map<String, Object> row = DbUtils.queryOne("select p.* from em_claimarchive p where p.claimarchiveid="+claimarchiveid);
	...
	public void update(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder select = new StringBuilder("update em_claimarchive set "
				...
				+ " where claimarchiveid=" + claimarchiveid + "" 
			);
			int result = DbUtils.save(select.toString());
	...
	public void list(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			StringBuilder countSql = new StringBuilder("select count(p.claimarchiveid) as total from em_claimarchive p where p.claimarchiveid>=0"
			...
			// 列表分页语句
			resultRows = DbUtils.query("select p.* from em_claimarchive p where p.claimarchiveid>=0"
					+ (keyword.length()>0?" and p.claimarchivename like '%" + keyword + "%'":"")
