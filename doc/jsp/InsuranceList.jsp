...
<table class="tlist">
	 <form action="/emInsurance.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>关键字</th>
...
<td>${item.insurancetype}</td>
				...
				<c:if test="${auth30 eq '1'}"><a href="/emInsurance.do?method=fill&insuranceid=${item.insuranceid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a>
					<a href="javascript:deleteInsurance(${item.insuranceid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a>
				</c:if>
				<c:if test="${auth30 ne '1'}"><a href="/emInsurance.do?method=fill&insuranceid=${item.insuranceid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emInsurance.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emInsurance.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid;
...
<form action="/emInsurance.do?method=delete" id="deleteForm" name="deleteForm" method="post">
