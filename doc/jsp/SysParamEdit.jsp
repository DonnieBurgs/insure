...
<title>系统参数<c:if test="${auth88 eq '1'}">修改</c:if><c:if test="${auth88 ne '1'}">查看</c:if></title>
</head>
<body id="right">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">系统参数<c:if test="${auth88 eq '1'}">修改</c:if><c:if test="${auth88 ne '1'}">查看</c:if></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<a href="javascript:history.back();" style="margin-left:100px;"><i class="iconfont">&#xe63d;</i>返回列表</a></div>
	<div class="operationRight f-r">
	</div>
</div>
<form id="commonForm" action="/emSysParam.do?method=update" method="post">
	<table class="tform">
<tr>
...
<td><input type="text" id="alarmnumcheckin" name="alarmnumcheckin" value="${fn:replace(item.alarmnumcheckin,'"','&quot;')}" size=50></td>
</tr>
<tr>
<td width="120" class="right">默认打卡提醒持续时间（分钟）：</td>
<td><input type="text" id="alarmtimecheckin" name="alarmtimecheckin" value="${fn:replace(item.alarmtimecheckin,'"','&quot;')}" size=50></td>
</tr>
<tr>
<td width="120" class="right">默认签到提醒次数：</td>
<td><input type="text" id="alarmnumsign" name="alarmnumsign" value="${fn:replace(item.alarmnumsign,'"','&quot;')}" size=50></td>
</tr>
<tr>
<td width="120" class="right">默认签到提醒持续时间（分钟）：</td>
<td><input type="text" id="alarmtimesign" name="alarmtimesign" value="${fn:replace(item.alarmtimesign,'"','&quot;')}" size=50></td>
</tr>
<tr>
<td width="120" class="right">默认脱岗提醒次数：</td>
<td><input type="text" id="alarmnumleave" name="alarmnumleave" value="${fn:replace(item.alarmnumleave,'"','&quot;')}" size=50></td>
</tr>
<tr>
<td width="120" class="right">默认脱岗提醒持续时间（分钟）：</td>
<td><input type="text" id="alarmtimeleave" name="alarmtimeleave" value="${fn:replace(item.alarmtimeleave,'"','&quot;')}" size=50></td>
</tr>

		<tr>
			<td colspan="2">
			<input id="sysparamid" name="sysparamid" type="hidden" value="${item.sysparamid}"/>

				<input id="uf_parentid" name="uf_parentid" type="hidden" value="${uf_parentid}"/>
				<input id="keyword" name="keyword" type="hidden" value="${keyword}"/>
				<input id="m" name="m" type="hidden" value="${m}"/>
				<input id="s" name="s" type="hidden" value="${s}"/>
				<c:if test="${auth88 eq '1'}">
				<input type="button" class="btn1" value=" 提  交 " onclick="checkf()"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
				<input type="button" class="btn1" value=" 删  除 " onclick="checkdelete()"/>
				</c:if>
				<c:if test="${auth88 ne '1'}">
...
<form id="deleteForm" action="/emSysParam.do?method=delete" method="post">
<input id="sysparamid" name="sysparamid" type="hidden" value="${item.sysparamid}"/>
<input id="uf_parentid" name="uf_parentid" type="hidden" value="${uf_parentid}"/>
<input id="keyword" name="keyword" type="hidden" value="${keyword}"/>
<input id="m" name="m" type="hidden" value="${m}"/>
