...
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">系统参数列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth88 eq '1'}"><a href="/emSysParam.do?method=blank&uf_parentid=${uf_parentid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增系统参数</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>
<table class="tlist">
	 <form action="/emSysParam.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>关键字</th>
...
<td>${item.alarmnumcheckin}</td>
<td>${item.alarmtimecheckin}</td>
<td>${item.alarmnumsign}</td>
<td>${item.alarmtimesign}</td>

				<td width="250" class="operationBtn">
				<c:if test="${auth88 eq '1'}"><a href="/emSysParam.do?method=fill&sysparamid=${item.sysparamid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a>
					<a href="javascript:deleteSysParam(${item.sysparamid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a>
				</c:if>
				<c:if test="${auth88 ne '1'}"><a href="/emSysParam.do?method=fill&sysparamid=${item.sysparamid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emSysParam.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emSysParam.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid;
...
<form action="/emSysParam.do?method=delete" id="deleteForm" name="deleteForm" method="post">
<input type="hidden" id="sysparamid" name="sysparamid" value="">
...
function deleteSysParam(sysparamid){
	if(confirm("是否删除会员资料？")) {
