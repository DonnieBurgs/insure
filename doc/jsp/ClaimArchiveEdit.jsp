...
<title>案卷<c:if test="${auth42 eq '1'}">修改</c:if><c:if test="${auth42 ne '1'}">查看</c:if></title>
</head>
<body id="right">
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">案卷<c:if test="${auth42 eq '1'}">修改</c:if><c:if test="${auth42 ne '1'}">查看</c:if></font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
...
<td><input type="text" id="acceptdate" name="acceptdate" readonly="readonly" value="${item.acceptdate_}" onclick="WdatePicker({maxDate:'2018-12-31', dateFmt:'yyyy-MM-dd'})" style="width: 70px;"/></td>
</tr>
<tr>
<td width="120" class="right">案卷号：</td>
<td><input type="text" id="claimarchivenumber" name="claimarchivenumber" value="${fn:replace(item.claimarchivenumber,'"','&quot;')}" size=50></td>
</tr>

		<tr>
			<td colspan="2">
			<input id="claimarchiveid" name="claimarchiveid" type="hidden" value="${item.claimarchiveid}"/>
...
<form id="deleteForm" action="/emClaimArchive.do?method=delete" method="post">
<input id="claimarchiveid" name="claimarchiveid" type="hidden" value="${item.claimarchiveid}"/>
<input id="uf_parentid" name="uf_parentid" type="hidden" value="${uf_parentid}"/>
<input id="keyword" name="keyword" type="hidden" value="${keyword}"/>
<input id="m" name="m" type="hidden" value="${m}"/>
