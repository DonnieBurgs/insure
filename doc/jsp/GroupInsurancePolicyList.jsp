...
<div class="operation o-h bg-f8">
	<div class="operationLeft f-l"><font style="margin-left:10px;font-size:14px; font-weight:bold; text-indent:14px; letter-spacing:2px;">团保方案列表</font>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<c:if test="${auth32 eq '1'}"><a href="/emGroupInsurancePolicy.do?method=blank&uf_parentid=${uf_parentid}&keyword=${keyword}&m=${m}&s=${s}" style="margin-left:100px;"><i class="iconfont">&#xe681;</i>新增团保方案</a></c:if></div>
	<div class="operationRight f-r">
	</div>
</div>
<table class="tlist">
	 <form action="/emGroupInsurancePolicy.do?method=list" id="searchForm" name="searchForm" method="post">
	 <thead>
	 <tr class="title">
	 <th width=10%>关键字</th>
...
<c:forEach var="item" items="${resultRows}">
			<tr>
<td>${item.applicantcompanyid}</td>
<td>${item.insurercompanyid}</td>
<td>${item.groupinsurancepolicyname}</td>
<td>${item.policynumber}</td>
<td>${item.premium}</td>

				<td width="250" class="operationBtn">
				<c:if test="${auth32 eq '1'}"><a href="/emGroupInsurancePolicy.do?method=fill&groupinsurancepolicyid=${item.groupinsurancepolicyid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看/编辑</a>
					<a href="javascript:deleteGroupInsurancePolicy(${item.groupinsurancepolicyid}, 1)" style="margin-left:10px;"><i class="iconfont">&#xe636;</i>删除</a>
				</c:if>
				<c:if test="${auth32 ne '1'}"><a href="/emGroupInsurancePolicy.do?method=fill&groupinsurancepolicyid=${item.groupinsurancepolicyid}&keyword=${keyword}&uf_parentid=${uf_parentid}&m=${m}&s=${s}" style="margin-left:0px;"><i class="iconfont">&#xe6d6;</i>查看</a>
				</c:if>
				</td>
			</tr>
</c:forEach>
...
<%@ include file="/inc/pagebar_s.jsp"%> <%
	...
	if(request.getAttribute("totalRow") != null){
		...
		if (_totalCount != null && _totalCount != 0) {
			...
			String u = "/emGroupInsurancePolicy.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid + "&m=" + pageSize;
			String u1 = "/emGroupInsurancePolicy.do?method=list&keyword=" + keyword + "&uf_parentid=" + uf_parentid;
...
<form action="/emGroupInsurancePolicy.do?method=delete" id="deleteForm" name="deleteForm" method="post">
<input type="hidden" id="groupinsurancepolicyid" name="groupinsurancepolicyid" value="">
...
function deleteGroupInsurancePolicy(groupinsurancepolicyid){
	if(confirm("是否删除会员资料？")) {
		$("#groupinsurancepolicyid").val(groupinsurancepolicyid);
		$("#t").val(t);
		deleteForm.submit();
